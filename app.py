...
# -------------------- Routing / modes --------------------
# Public deployment has two important URLs:
#   ?view=play  -> students play the game
#   ?view=host  -> your laptop shows the live leaderboard + QR code
# This avoids relying on the sidebar, which is often hidden/collapsed on phones.
view = str(st.query_params.get("view", "")).lower().strip()
mode_options = ["Player Game", "Host Screen + Live Leaderboard", "Reset Leaderboard", "Answer Key"]
...
if view == "host":
    ...
    # Local/testing fallback. On the public event link, use ?view=host for host mode.
...
<p>For the event, deploy this app online, then paste/set the public app URL below. Students do <b>not</b> need to be on the same Wi‑Fi.</p>
<p class='small-note'>Recommended: Streamlit Community Cloud. Set <b>PUBLIC_APP_URL</b> in Streamlit Secrets, or paste the link here during testing.</p>
</div>
""", unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
        ...
        with st.expander("Show links", expanded=False):
            ...
            st.code(player_url)
            st.write("Host leaderboard link:")
            st.code(host_url)
